package com.github.tedblair2.muziki3.features.details.viewmodel

import android.content.Context
import android.net.Uri
import androidx.core.content.ContextCompat
import androidx.lifecycle.ViewModel
import androidx.lifecycle.viewModelScope
import androidx.media3.common.MediaItem
import androidx.media3.common.MediaMetadata
import androidx.media3.session.MediaController
import com.github.tedblair2.muziki3.core.local.AppCoroutineContext
import com.github.tedblair2.muziki3.core.local.RoomRepository
import com.github.tedblair2.muziki3.core.local.datastore.DataPrefService
import com.github.tedblair2.muziki3.core.local.model.Audio
import com.github.tedblair2.muziki3.data.redux.Store
import com.github.tedblair2.muziki3.features.details.ui.DetailScreenType
import com.github.tedblair2.muziki3.features.details.ui.DetailsScreenState
import com.google.common.util.concurrent.ListenableFuture
import dagger.hilt.android.lifecycle.HiltViewModel
import dagger.hilt.android.qualifiers.ApplicationContext
import kotlinx.coroutines.flow.MutableStateFlow
import kotlinx.coroutines.flow.asStateFlow
import kotlinx.coroutines.flow.flowOn
import kotlinx.coroutines.flow.update
import kotlinx.coroutines.launch
import javax.inject.Inject

@HiltViewModel
class DetailsViewModel @Inject constructor(
    private val store: Store ,
    private val controllerFuture: ListenableFuture<MediaController> ,
    @ApplicationContext private val context: Context,
    private val roomRepository: RoomRepository,
    private val appCoroutineContext: AppCoroutineContext,
    private val dataPrefService: DataPrefService
):ViewModel() {

    private val _detailsScreenState= MutableStateFlow(DetailsScreenState())
    val detailsScreenState = _detailsScreenState.asStateFlow()
    private var mediaController: MediaController?=null

    init {
        initMediaController()
        getCurrentTheme()
    }

    fun onEvent(event: DetailsScreenEvents){
        when(event){
            is DetailsScreenEvents.GetAlbumSongs -> {
                viewModelScope.launch {
                    store.getCurrentState {  }
                        .collect {appState->
                            _detailsScreenState.update {
                                it.copy(
                                    currentScreenType = DetailScreenType.ALBUM,
                                    songs = getSongsForAlbum(appState.songs,event.albumName),
                                    isLoading = false,
                                    title = "",
                                    isMiniPlayerVisible = appState.playerScreenState.isMiniPlayerVisible
                                )
                            }
                        }
                }
            }
            DetailsScreenEvents.GetFavoriteSongs -> {
                getFavoriteSongs()
                setMiniPlayerVisibility()
            }
            is DetailsScreenEvents.GetPlaylistSongs -> {
                getPlaylistSongs(event.playlistId)
                setMiniPlayerVisibility()
            }
            DetailsScreenEvents.GetRecentSongs -> {
                viewModelScope.launch {
                    store.getCurrentState {  }
                        .collect{appState->
                            _detailsScreenState.update {
                                it.copy(
                                    currentScreenType = DetailScreenType.RECENT,
                                    songs = getRecentSongs(appState.songs),
                                    isLoading = false,
                                    title = "Recently Added",
                                    isMiniPlayerVisible = appState.playerScreenState.isMiniPlayerVisible
                                )
                            }
                        }
                }
            }
            is DetailsScreenEvents.OnAudioClick->{
                kotlin.run {
                    val player = this.mediaController ?: return@run
                    player.setMediaItems(setMediaItems(event.songs), event.position, 0)
                    player.prepare()
                    player.play()
                    updateRecentPlaylist(event.songs)
                    updateLastPosition(event.position)
                }
            }
            is DetailsScreenEvents.OnRemoveFavorite->{
                removeFromFavorite(event.audio)
            }
            is DetailsScreenEvents.OnAddToFavorite->{
                addToFavorite(event.audio)
            }
            is DetailsScreenEvents.OnRemoveFromPlaylist -> {
                removeFromPlaylist(event.audio)
            }

            is DetailsScreenEvents.RemoveAudioFromPlayer -> {
                removeAudioFromPlayer(event.audio)
                removeAudioFromFavoriteAndPlaylist(event.audio)
            }
        }
    }

    private fun setMiniPlayerVisibility(){
        viewModelScope.launch {
            store.getCurrentState {  }
                .collect{appState->
                    _detailsScreenState.update {
                        it.copy(isMiniPlayerVisible = appState.playerScreenState.isMiniPlayerVisible)
                    }
                }
        }
    }

    private fun getCurrentTheme(){
        viewModelScope.launch {
            dataPrefService.getCurrentTheme()
                .collect{currentTheme->
                    _detailsScreenState.update {
                        it.copy(
                            currentTheme = currentTheme
                        )
                    }
                }
        }
    }

    private fun removeAudioFromPlayer(audio: Audio){
        viewModelScope.launch {
            roomRepository.getAudioRecentList().collect{recents->
                if (recents.isNotEmpty()){
                    recents.forEachIndexed { index, audioRecent ->
                        if (audio.id==audioRecent.id){
                            removeMediaItem(index)
                        }
                    }
                }
            }
        }
    }

    private fun removeAudioFromFavoriteAndPlaylist(audio: Audio){
        viewModelScope.launch(appCoroutineContext.io) {
            roomRepository.deleteAudioFromFavoritesAndPlaylist(audio)
        }
    }

    private fun removeMediaItem(position: Int){
        kotlin.run {
            val player = this.mediaController ?: return@run
            player.removeMediaItem(position)
        }
    }

    private fun updateLastPosition(position:Int){
        viewModelScope.launch(appCoroutineContext.io) {
            dataPrefService.setLastPosition(position.toLong())
        }
    }

    private fun updateRecentPlaylist(songs: List<Audio>){
        viewModelScope.launch(appCoroutineContext.io) {
            roomRepository.addAudioRecentListToDb(songs)
        }
    }

    private fun getPlaylistSongs(id:Int){
        viewModelScope.launch {
            roomRepository.getPlaylistById(id)
                .flowOn(appCoroutineContext.io)
                .collect{playlist->
                    _detailsScreenState.update {
                        it.copy(
                            currentScreenType = DetailScreenType.PLAYLIST,
                            songs = playlist.songs,
                            isLoading = false,
                            title = playlist.name,
                            playlist = playlist
                        )
                    }
                }
        }
    }

    private fun removeFromPlaylist(audio: Audio){
        viewModelScope.launch(appCoroutineContext.io) {
            detailsScreenState.value.playlist?.let {
                roomRepository.deleteAudioFromPlaylist(audio,it)
            }
        }
    }

    private fun getFavoriteSongs(){
        viewModelScope.launch {
            roomRepository.getFavorites()
                .flowOn(appCoroutineContext.io)
                .collect{audioList->
                    _detailsScreenState.update {
                        it.copy(
                            currentScreenType = DetailScreenType.FAVORITE,
                            songs = audioList,
                            isLoading = false,
                            title = "Favorites"
                        )
                    }
                }
        }
    }

    private fun addToFavorite(audio: Audio){
        viewModelScope.launch(appCoroutineContext.io) {
            roomRepository.addAudioToFavorites(audio)
        }
    }

    private fun removeFromFavorite(audio: Audio){
        viewModelScope.launch(appCoroutineContext.io) {
            roomRepository.deleteAudioFromFavorites(audio)
        }
    }

    private fun initMediaController(){
        controllerFuture.addListener({
            mediaController=try {
                controllerFuture.get()
            }catch (e:Exception){
                null
            }
        }, ContextCompat.getMainExecutor(context))
    }

    private fun getRecentSongs(audioList:List<Audio>):List<Audio>{
        val list= arrayListOf<Audio>()
        if (audioList.size<10){
            return audioList
        }else{
            var j=0
            while (j<10){
                list.add(audioList[j])
                j++
            }
        }
        return list
    }

    private fun getSongsForAlbum(audioList:List<Audio> , name:String):ArrayList<Audio>{
        val list= arrayListOf<Audio>()
        var j=0
        for (song in audioList){
            if (song.album == name){
                list.add(j,song)
                j++
            }
        }
        return getUniqueSongs(list)
    }

    private fun getUniqueSongs(list:ArrayList<Audio>):ArrayList<Audio>{
        val set= list.toSet()
        return ArrayList(set)
    }

    private fun setMediaItems(songs:List<Audio>):List<MediaItem>{
        return songs.map {
            MediaItem.Builder()
                .setMediaMetadata(getMediaMetadata(it))
                .setUri(Uri.parse(it.path))
                .build()
        }
    }

    private fun getMediaMetadata(song: Audio): MediaMetadata {
        return MediaMetadata.Builder()
            .setTitle(song.name)
            .setArtist(song.artist)
            .setAlbumTitle(song.album)
            .setArtworkUri(Uri.parse(song.path))
            .setArtworkData(song.artWork, MediaMetadata.PICTURE_TYPE_FILE_ICON)
            .build()
    }
}